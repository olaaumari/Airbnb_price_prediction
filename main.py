...
st.write('''
# Application de Prévision des prix Airbnb sur la ville de Nice et ses alentours
''')
...
#text_input = st.text_input("Entré un prix pour afficher les biens disponibles")
#st.markdown("Vous avez choisis les biens au prix de : " + text_input)
number_input = st.number_input(
    "Entré un prix pour afficher les biens disponibles ", min_value=0, max_value=20000, step=1
)
nombre_annonces = df[df['price']==number_input].shape[0]
st.write("Il y a : " , nombre_annonces , " annonces à ce prix")

st.write(df[df['price']==number_input])
#st.write(df.shape)
...
st.write("Prix estimé : ", prix(reg_rf))
